...
function getNextDocumentNumber() {
    const row = db.prepare(`
        ...
        FROM petty_expenses_shipping_clearance
...
function register() {
    ipcMain.handle('get-next-shipping-clearance-expense-number', () => {
        try {
    ...
    ipcMain.handle('get-shipping-clearance-expenses', (event, params = {}) => {
        try {
            ...
            const total = db.prepare(`SELECT COUNT(*) as count FROM petty_expenses_shipping_clearance ${whereSql}`).get(args).count || 0;
            const totalAmount = db.prepare(`SELECT COALESCE(SUM(amount), 0) as totalAmount FROM petty_expenses_shipping_clearance ${whereSql}`).get(args).totalAmount || 0;
    ...
    ipcMain.handle('save-shipping-clearance-expense', (event, data = {}) => {
        try {
            ...
            if (amount <= 0) {
                return { success: false, error: 'قيمة الشحن و التخليص غير صحيحة' };
            }
            ...
            if (amount > currentBalance) {
                return { success: false, error: 'قيمة الشحن و التخليص أكبر من رصيد الخزينة المتاح' };
            }

            const tx = db.transaction(() => {
                const documentNumber = getNextDocumentNumber();
                const expenseInfo = db.prepare(`
                    INSERT INTO petty_expenses_shipping_clearance (document_number, expense_date, amount, statement, notes)
                ...
                const treasuryInfo = db.prepare(`
                    ...
                    VALUES ('expense', @amount, @transaction_date, @description, @related_invoice_id, 'petty_expenses_shipping_clearance')
                `).run({
                    amount,
                    transaction_date: expenseDate,
                    description: `شحن و تخليص ${documentNumber} - ${statement}`,
                    related_invoice_id: expenseId
                });

                db.prepare('UPDATE petty_expenses_shipping_clearance SET treasury_transaction_id = ? WHERE id = ?').run(treasuryInfo.lastInsertRowid, expenseId);
    ...
    ipcMain.handle('update-shipping-clearance-expense', (event, data = {}) => {
        try {
            const expenseId = Number(data.id);
            if (!expenseId) {
                return { success: false, error: 'معرف الشحن و التخليص غير صحيح' };
            }
            ...
            if (!existing) {
                return { success: false, error: 'لم يتم العثور على مستند الشحن و التخليص' };
            }
            ...
            const tx = db.transaction(() => {
                db.prepare(`
                    UPDATE petty_expenses_shipping_clearance
                ...
                const description = `شحن و تخليص ${existing.document_number} - ${statement}`;
    ...
    ipcMain.handle('delete-shipping-clearance-expense', (event, expenseId) => {
        try {
            ...
            const tx = db.transaction(() => {
                db.prepare('DELETE FROM petty_expenses_shipping_clearance WHERE id = ?').run(id);
                if (existing.treasury_transaction_id) {
    ...
    ipcMain.handle('save-shipping-clearance-expenses-pdf', async (event, payload = {}) => {
        try {
            ...
            const defaultName = String(payload.defaultName || `Shipping_Clearance_Expenses_${date}.pdf`).replace(/[<>:"/\\|?*\x00-\x1F]/g, '_');
            const defaultPath = path.join(app.getPath('documents'), defaultName);
            const { canceled, filePath } = await dialog.showSaveDialog(win, {
                title: 'حفظ ملف الشحن و التخليص PDF',
                defaultPath,
