...
function register() {
    ipcMain.handle('get-next-petty-expense-number', () => {
        try {
    ...
    ipcMain.handle('get-petty-expenses', (event, params = {}) => {
        try {
    ...
    ipcMain.handle('save-petty-expense', (event, data = {}) => {
        try {
            ...
            if (amount <= 0) {
                return { success: false, error: 'قيمة النثريات غير صحيحة' };
            }
            ...
            if (amount > currentBalance) {
                return { success: false, error: 'قيمة النثريات أكبر من رصيد الخزينة المتاح' };
            }

            const tx = db.transaction(() => {
                const documentNumber = getNextDocumentNumber();
                const expenseInfo = db.prepare(`
                    INSERT INTO petty_expenses (document_number, expense_date, amount, statement, notes)
                ...
                `).run({
                    ...
                    description: `نثريات ${documentNumber} - ${statement}`,
    ...
    ipcMain.handle('update-petty-expense', (event, data = {}) => {
        try {
            const expenseId = Number(data.id);
            if (!expenseId) {
                return { success: false, error: 'معرف النثريات غير صحيح' };
            }
            ...
            if (!existing) {
                return { success: false, error: 'لم يتم العثور على مستند النثريات' };
            }
            ...
            const tx = db.transaction(() => {
                ...
                const description = `نثريات ${existing.document_number} - ${statement}`;
    ...
    ipcMain.handle('delete-petty-expense', (event, expenseId) => {
        try {
    ...
    ipcMain.handle('save-petty-expenses-pdf', async (event, payload = {}) => {
        try {
            ...
            const defaultName = String(payload.defaultName || `Petty_Expenses_${date}.pdf`).replace(/[<>:"/\\|?*\x00-\x1F]/g, '_');
            const defaultPath = path.join(app.getPath('documents'), defaultName);
            const { canceled, filePath } = await dialog.showSaveDialog(win, {
                title: 'حفظ ملف النثريات PDF',
                defaultPath,
