...
function getNextDocumentNumber() {
    const row = db.prepare(`
        ...
        FROM factory_petty_expenses
        WHERE document_number GLOB 'FNTH-[0-9]*'
        ORDER BY CAST(SUBSTR(document_number, 6) AS INTEGER) DESC
        LIMIT 1
    `).get();

    const lastNumber = row ? Number(String(row.document_number).slice(5)) : 0;
    return `FNTH-${String((Number.isFinite(lastNumber) ? lastNumber : 0) + 1).padStart(4, '0')}`;
}
...
const CATEGORIES = [
    { id: 'factory', prefix: 'factory-petty', title: 'نثريات المصنع' }
];
    ...
    CATEGORIES.forEach(({ id: categoryId, prefix, title }) => {
        ...
        ipcMain.handle(`get-${prefix}-expenses`, (event, params = {}) => {
            try {
                ...
                const total = db.prepare(`SELECT COUNT(*) as count FROM factory_petty_expenses ${whereSql}`).get(args).count || 0;
                const totalAmount = db.prepare(`SELECT COALESCE(SUM(amount), 0) as totalAmount FROM factory_petty_expenses ${whereSql}`).get(args).totalAmount || 0;
            ...
            try {
                ...
                const tx = db.transaction(() => {
                    ...
                    const expenseInfo = db.prepare(`
                        INSERT INTO factory_petty_expenses (category, document_number, expense_date, amount, statement, notes)
                    ...
                    db.prepare('UPDATE factory_petty_expenses SET treasury_transaction_id = ? WHERE id = ?').run(treasuryInfo.lastInsertRowid, expenseId);
        ...
        ipcMain.handle(`delete-${prefix}-expense`, (event, expenseId) => {
            try {
                ...
                const tx = db.transaction(() => {
                    db.prepare('DELETE FROM factory_petty_expenses WHERE id = ?').run(id);
                    if (existing.treasury_transaction_id) {
