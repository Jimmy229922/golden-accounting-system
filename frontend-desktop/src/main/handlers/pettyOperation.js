...
function register() {
    ipcMain.handle('get-next-operation-expense-number', () => {
        try {
    ...
    ipcMain.handle('get-operation-expenses', (event, params = {}) => {
        try {
            ...
            const total = db.prepare(`SELECT COUNT(*) as count FROM petty_expenses_operation ${whereSql}`).get(args).count || 0;
            const totalAmount = db.prepare(`SELECT COALESCE(SUM(amount), 0) as totalAmount FROM petty_expenses_operation ${whereSql}`).get(args).totalAmount || 0;
    ...
    ipcMain.handle('save-operation-expense', (event, data = {}) => {
        try {
            ...
            if (amount <= 0) {
                return { success: false, error: 'قيمة التشغيل غير صحيحة' };
            }
            ...
            if (amount > currentBalance) {
                return { success: false, error: 'قيمة التشغيل أكبر من رصيد الخزينة المتاح' };
            }

            const tx = db.transaction(() => {
                const documentNumber = getNextDocumentNumber();
                const expenseInfo = db.prepare(`
                    INSERT INTO petty_expenses_operation (document_number, expense_date, amount, statement, notes)
                ...
                const treasuryInfo = db.prepare(`
                    ...
                    VALUES ('expense', @amount, @transaction_date, @description, @related_invoice_id, 'petty_expenses_operation')
                `).run({
                    amount,
                    transaction_date: expenseDate,
                    description: `تشغيل ${documentNumber} - ${statement}`,
                    related_invoice_id: expenseId
                });

                db.prepare('UPDATE petty_expenses_operation SET treasury_transaction_id = ? WHERE id = ?').run(treasuryInfo.lastInsertRowid, expenseId);
    ...
    ipcMain.handle('update-operation-expense', (event, data = {}) => {
        try {
            const expenseId = Number(data.id);
            if (!expenseId) {
                return { success: false, error: 'معرف التشغيل غير صحيح' };
            }
            ...
            if (!existing) {
                return { success: false, error: 'لم يتم العثور على مستند التشغيل' };
            }
            ...
            const tx = db.transaction(() => {
                db.prepare(`
                    UPDATE petty_expenses_operation
                ...
                const description = `تشغيل ${existing.document_number} - ${statement}`;
    ...
    ipcMain.handle('delete-operation-expense', (event, expenseId) => {
        try {
            ...
            const tx = db.transaction(() => {
                db.prepare('DELETE FROM petty_expenses_operation WHERE id = ?').run(id);
                if (existing.treasury_transaction_id) {
    ...
    ipcMain.handle('save-operation-expenses-pdf', async (event, payload = {}) => {
        try {
            ...
            const defaultName = String(payload.defaultName || `Operation_Expenses_${date}.pdf`).replace(/[<>:"/\\|?*\x00-\x1F]/g, '_');
            const defaultPath = path.join(app.getPath('documents'), defaultName);
            const { canceled, filePath } = await dialog.showSaveDialog(win, {
                title: 'حفظ ملف التشغيل PDF',
                defaultPath,
