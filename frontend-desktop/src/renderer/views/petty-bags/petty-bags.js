        ...
        <main class="petty-page">
            <section class="petty-hero">
                ...
                <div class="hero-content">
                    <h1>الشكاير</h1>
                    <p>متابعة مصروفات الشكاير وخدماتها التشغيلية أولًا بأول</p>
                </div>
            ...
            <section class="stats-container petty-stats">
                <div class="stat-card stat-expense">
                    ...
                    <div class="stat-info">
                        <div class="stat-title">إجمالي الشكاير</div>
            ...
            <section class="invoice-form-container print-container">
                <div class="invoice-shell" style="padding-bottom: 10px;">
                    <div class="form-title-row" style="border-bottom: none; padding-bottom: 0;">
                        <h2 class="form-title">كشف الشكاير</h2>
                    </div>
            ...
            <div class="petty-modal-card" role="dialog" aria-modal="true" aria-labelledby="pettyModalTitle">
                <div class="petty-modal-header">
                    <div class="petty-modal-title">
                        ...
                        <h2 id="pettyModalTitle">إضافة مستند الشكاير</h2>
                    </div>
...
async function refreshDocumentNumber() {
    const result = await window.electronAPI.getNextBagsExpenseNumber();
...
async function loadExpenses() {
    const result = await window.electronAPI.getBagsExpenses(getFilters());
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر تحميل الشكاير', 'error');
        return;
    }
...
async function openEditModal(id) {
    ...
    if (!row) {
        showMessage('تعذر العثور على مستند الشكاير', 'error');
        return;
    }
...
async function deleteExpense(id) {
    ...
    const confirmed = await window.showConfirmDialog('هل تريد حذف مستند الشكاير؟');
    if (!confirmed) {
        return;
    }

    const result = await window.electronAPI.deleteBagsExpense(Number(id));
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر حذف مستند الشكاير', 'error');
        return;
    }

    showMessage('تم حذف مستند الشكاير بنجاح', 'success');
    await loadExpenses();
}
...
async function saveExpense(event) {
    ...
    if (state.editingId) {
        ...
        const result = await window.electronAPI.updateBagsExpense(payload);
        if (!result || !result.success) {
            showMessage((result && result.error) || 'تعذر تعديل الشكاير', 'error');
            return;
        }

        showMessage('تم تعديل الشكاير بنجاح', 'success');
        state.editingId = null;
    ...
    const result = await window.electronAPI.saveBagsExpense(payload);
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر حفظ الشكاير', 'error');
        return;
    }

    showMessage('تم حفظ الشكاير بنجاح', 'success');
...
async function exportPdf() {
    ...
    try {
        const date = today();
        const result = await window.electronAPI.saveBagsExpensesPdf({ defaultName: `Bags_Expenses_${date}.pdf` });
        if (result && result.success) {
