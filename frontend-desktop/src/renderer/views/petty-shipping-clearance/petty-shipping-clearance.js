            ...
            <section class="invoice-form-container print-container">
                <div class="invoice-shell" style="padding-bottom: 10px;">
                    <div class="form-title-row" style="border-bottom: none; padding-bottom: 0;">
                        <h2 class="form-title">كشف الشحن و التخليص</h2>
                    </div>
                    
                    <div class="petty-print-title">
                        <h2>كشف الشحن و التخليص</h2>
                        <p id="printRange"></p>
                    </div>
...
async function refreshDocumentNumber() {
    const result = await window.electronAPI.getNextShippingClearanceExpenseNumber();
...
async function exportPdf() {
    ...
    try {
        const date = today();
        const result = await window.electronAPI.saveShippingClearanceExpensesPdf({ defaultName: `Shipping_Clearance_Expenses_${date}.pdf` });
        if (result && result.success) {
