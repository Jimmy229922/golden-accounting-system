        ...
        <main class="petty-page">
            <section class="petty-hero">
                ...
                <div class="hero-content">
                    <h1>التشغيل</h1>
                    <p>مصروفات التشغيل اليومية للخطوط والمعدات دون انقطاع</p>
                </div>
            ...
            <section class="stats-container petty-stats">
                <div class="stat-card stat-expense">
                    ...
                    <div class="stat-info">
                        <div class="stat-title">إجمالي التشغيل</div>
            ...
            <section class="invoice-form-container print-container">
                <div class="invoice-shell" style="padding-bottom: 10px;">
                    <div class="form-title-row" style="border-bottom: none; padding-bottom: 0;">
                        <h2 class="form-title">كشف التشغيل</h2>
                    </div>
            ...
            <div class="petty-modal-card" role="dialog" aria-modal="true" aria-labelledby="pettyModalTitle">
                <div class="petty-modal-header">
                    <div class="petty-modal-title">
                        ...
                        <h2 id="pettyModalTitle">إضافة مستند التشغيل</h2>
                    </div>
...
async function refreshDocumentNumber() {
    const result = await window.electronAPI.getNextOperationExpenseNumber();
...
async function loadExpenses() {
    const result = await window.electronAPI.getOperationExpenses(getFilters());
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر تحميل التشغيل', 'error');
        return;
    }
...
function renderRows() {
    ...
    if (!state.rows.length) {
        body.innerHTML = `<tr><td colspan="6" class="petty-empty">لا توجد التشغيل مسجلة</td></tr>`;
        return;
    }
...
async function openEditModal(id) {
    ...
    if (!row) {
        showMessage('تعذر العثور على مستند التشغيل', 'error');
        return;
    }
...
async function deleteExpense(id) {
    ...
    const result = await window.electronAPI.deleteOperationExpense(Number(id));
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر حذف مستند التشغيل', 'error');
        return;
    }

    showMessage('تم حذف مستند التشغيل بنجاح', 'success');
    await loadExpenses();
}
...
async function saveExpense(event) {
    ...
    try {
        if (state.editingId) {
            ...
            const result = await window.electronAPI.updateOperationExpense(payload);
            if (!result || !result.success) {
                showMessage((result && result.error) || 'تعذر تعديل التشغيل', 'error');
                return;
            }

            showMessage('تم تعديل التشغيل بنجاح', 'success');
            state.editingId = null;
        ...
        const result = await window.electronAPI.saveOperationExpense(payload);
        if (!result || !result.success) {
            showMessage((result && result.error) || 'تعذر حفظ التشغيل', 'error');
            return;
        }

        showMessage('تم حفظ التشغيل بنجاح', 'success');
        document.getElementById('addExpenseModal').classList.add('hidden');
        document.getElementById('pettyForm').reset();
        document.getElementById('expenseDate').value = today();
        await refreshDocumentNumber();
...
async function exportPdf() {
    ...
    const exportResult = await window.electronAPI.getOperationExpenses({
        page: 1,
        pageSize: 100000,
        startDate,
        endDate
    ...
    try {
        const date = today();
        const result = await window.electronAPI.saveOperationExpensesPdf({ defaultName: `Operation_Expenses_${date}.pdf` });
        if (result && result.success) {
