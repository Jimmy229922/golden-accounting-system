        ...
        <main class="petty-page">
            <section class="petty-hero">
                ...
                <div class="hero-content">
                    <h1>الفحص</h1>
                    <p>تسجيل مصروفات الفحص والجودة لكل شحنة بدقة</p>
                </div>
            ...
            <section class="stats-container petty-stats">
                <div class="stat-card stat-expense">
                    ...
                    <div class="stat-info">
                        <div class="stat-title">إجمالي الفحص</div>
            ...
            <section class="invoice-form-container print-container">
                <div class="invoice-shell" style="padding-bottom: 10px;">
                    <div class="form-title-row" style="border-bottom: none; padding-bottom: 0;">
                        <h2 class="form-title">كشف الفحص</h2>
                    </div>
            ...
            <div class="petty-modal-card" role="dialog" aria-modal="true" aria-labelledby="pettyModalTitle">
                <div class="petty-modal-header">
                    <div class="petty-modal-title">
                        ...
                        <h2 id="pettyModalTitle">إضافة مستند الفحص</h2>
                    </div>
...
async function refreshDocumentNumber() {
    const result = await window.electronAPI.getNextInspectionExpenseNumber();
...
async function loadExpenses() {
    const result = await window.electronAPI.getInspectionExpenses(getFilters());
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر تحميل الفحص', 'error');
        return;
    }
...
function renderRows() {
    ...
    if (!state.rows.length) {
        body.innerHTML = `<tr><td colspan="6" class="petty-empty">لا توجد الفحص مسجلة</td></tr>`;
        return;
    }
...
async function openEditModal(id) {
    ...
    if (!row) {
        showMessage('تعذر العثور على مستند الفحص', 'error');
        return;
    }
...
async function deleteExpense(id) {
    ...
    const confirmed = await window.showConfirmDialog('هل تريد حذف مستند الفحص؟');
    if (!confirmed) {
        return;
    }

    const result = await window.electronAPI.deleteInspectionExpense(Number(id));
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر حذف مستند الفحص', 'error');
        return;
    }

    showMessage('تم حذف مستند الفحص بنجاح', 'success');
    await loadExpenses();
}
...
async function exportPdf() {
    ...
    const exportResult = await window.electronAPI.getInspectionExpenses({
        page: 1,
        pageSize: 100000,
        startDate,
        endDate
    ...
    try {
        const date = today();
        const result = await window.electronAPI.saveInspectionExpensesPdf({ defaultName: `Inspection_Expenses_${date}.pdf` });
        if (result && result.success) {
