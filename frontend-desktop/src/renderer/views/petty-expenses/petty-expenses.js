        ...
        <main class="petty-page">
            <section class="petty-hero">
                ...
                <div class="hero-content">
                    <h1>النثريات</h1>
                    <p>تسجيل ومتابعة المصاريف اليومية الخارجة من المكتب</p>
                </div>
            ...
            <section class="stats-container petty-stats">
                <div class="stat-card stat-expense">
                    ...
                    <div class="stat-info">
                        <div class="stat-title">إجمالي النثريات</div>
            ...
            <section class="invoice-form-container print-container">
                <div class="invoice-shell" style="padding-bottom: 10px;">
                    <div class="form-title-row" style="border-bottom: none; padding-bottom: 0;">
                        <h2 class="form-title">كشف النثريات</h2>
                    </div>
            ...
            <div class="petty-modal-card" role="dialog" aria-modal="true" aria-labelledby="pettyModalTitle">
                <div class="petty-modal-header">
                    <div class="petty-modal-title">
                        ...
                        <h2 id="pettyModalTitle">إضافة مستند نثريات</h2>
                    </div>
...
async function refreshDocumentNumber() {
    const result = await window.electronAPI.getNextPettyExpenseNumber();
...
async function loadExpenses() {
    const result = await window.electronAPI.getPettyExpenses(getFilters());
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر تحميل النثريات', 'error');
        return;
    }
...
function renderRows() {
    ...
    if (!state.rows.length) {
        body.innerHTML = `<tr><td colspan="6" class="petty-empty">لا توجد نثريات مسجلة</td></tr>`;
        return;
    }
...
async function openEditModal(id) {
    ...
    if (!row) {
        showMessage('تعذر العثور على مستند النثريات', 'error');
        return;
    }
...
async function deleteExpense(id) {
    ...
    const confirmed = await window.showConfirmDialog('هل تريد حذف مستند النثريات؟');
    if (!confirmed) {
        return;
    }

    const result = await window.electronAPI.deletePettyExpense(Number(id));
    if (!result || !result.success) {
        showMessage((result && result.error) || 'تعذر حذف مستند النثريات', 'error');
        return;
    }

    showMessage('تم حذف مستند النثريات بنجاح', 'success');
    await loadExpenses();
}
...
async function saveExpense(event) {
    ...
    try {
        if (state.editingId) {
            ...
            const result = await window.electronAPI.updatePettyExpense(payload);
            if (!result || !result.success) {
                showMessage((result && result.error) || 'تعذر تعديل النثريات', 'error');
                return;
            }

            showMessage('تم تعديل النثريات بنجاح', 'success');
            state.editingId = null;
        ...
        const result = await window.electronAPI.savePettyExpense(payload);
        if (!result || !result.success) {
            showMessage((result && result.error) || 'تعذر حفظ النثريات', 'error');
            return;
        }

        showMessage('تم حفظ النثريات بنجاح', 'success');
        document.getElementById('addExpenseModal').classList.add('hidden');
        document.getElementById('pettyForm').reset();
        document.getElementById('expenseDate').value = today();
        await refreshDocumentNumber();
...
async function exportPdf() {
    ...
    const exportResult = await window.electronAPI.getPettyExpenses({
        page: 1,
        pageSize: 100000,
        startDate,
        endDate
    ...
    try {
        const date = today();
        const result = await window.electronAPI.savePettyExpensesPdf({ defaultName: `Petty_Expenses_${date}.pdf` });
        if (result && result.success) {
